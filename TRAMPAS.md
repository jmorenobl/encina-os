...
| **70** | `qemu-aarch64-softmmu: failed to load ".../Data/Image"` con el `Image` perfecto (magia `ARMd` en 0x38, gzip íntegro, permisos bien) | El QEMU de UTM está sandboxeado: sólo abre ficheros que el bundle le declara; `-kernel`/`-initrd` de `AdditionalArguments` exigen `Image` e `initrd` TAMBIÉN como unidades (`ImageType CD`, `Interface None`) en `config.plist` | La receta de instalación desatendida de §4.84a (cirugía del bundle); fabricar-vm-medio.py + arranque directo del núcleo | [SCRIPTS.md](SCRIPTS.md) («70.», sección del 2026-09-01); MEDICIONES.md §4.88 |
...
último —hoy, la 71— y `bancos/enlaces.sh` comprueba que sus `§` y sus enlaces
resuelven.*
