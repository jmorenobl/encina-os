...
| D24 | **LOS TRES RELOJES, Y NINGUNA «EDICIÓN». Decisión de Jorge, 2026-08-31** (*«Adelante con la recomendación. Confío en tu criterio experto»*, sobre las opciones y precios escritos en [tareas/actualizacion.md](tareas/actualizacion.md), casilla A1). **La versión del producto es la de `encina-meta`** —lo que `bancos/versiones.sh` ya enunciaba y ahora es regla—, y los tres relojes se mueven así: **(1) un `.deb` nuevo sube `encina-meta` en menor** («0.2.1 → 0.2.2»: actualización de paquetes; con el canal de D25, la máquina instalada la recibe por `apt` sin reinstalar); **(2) una base nueva (la LTS siguiente) sube `encina-meta` en mayor**, y es lo único que se llama «versión nueva del sistema operativo»; **(3) TODA refabricación del medio sube la versión — la «edición» NO existe.** Refabricar sobre un *point release* nuevo de la base es una versión nueva del producto, aunque ningún `.deb` de Encina cambie. **La regla de §7 del 2026-08-29 queda CONFIRMADA tal cual** (un medio nuevo es una `encina-meta` nueva, una carpeta nueva y una etiqueta nueva; nada publicado cambia), sin enmienda. **Y la serie de la base va en el TÍTULO de toda release desde ya** —«Encina OS <versión> (Ubuntu <serie>)»—, no solo al saltar de mayor: quien lee «0.2.2 (Ubuntu 24.04)» sabe que no reinstala, y quien lea «1.0 (Ubuntu 26.04)» sabrá que hay un salto de base debajo. La serie **no se escribe a mano**: vive como literal `SERIE_BASE` en `imagen/sacar-version.sh` y `bancos/versiones.sh` la coteja contra la ISO oficial anclada del `Makefile`, con su control rojo (§4.87). Firefox es un cuarto reloj **ajeno**: flota desde Mozilla y no entra en la versión | **Se eligió la opción «más simple» sobre la «más honesta» a sabiendas, y este es el porqué.** La alternativa era admitir la «edición» (mismo número, huella nueva) con un segundo identificador; su precio es **un segundo número que enseñar, custodiar y vigilar** para distinguir algo que la huella y la carpeta de SourceForge ya distinguen — y un usuario no debe necesitar dos números para saber si reinstala. Lo que la opción simple cuesta, sin maquillar: `encina-meta` subirá de versión **sin que cambie nada suyo** cuando la causa sea un *point release* de la base o una refabricación (el `changelog` lo dirá con esa frase, que es lo honesto que queda). **Por qué la serie en el título y no en el Volume id:** el Volume id ya es la versión de `encina-meta` (`bancos/versiones.sh`) y es un campo corto y ya ocupado de la ISO; el título de la release no tiene esa estrechez y es lo primero que el usuario lee. **Vigilancia con control, no frase:** la casilla A1 exigía que `versiones.sh` vigile «ese segundo número»; se implementa cotejando el literal de `sacar-version.sh` contra el nombre de la ISO oficial del `Makefile` — si un día la base ancla `24.04.5` o `26.04` y el título no se toca, el banco lo pone en rojo (§4.87). **Qué reabriría D24:** que las refabricaciones se vuelvan tan frecuentes (una por *point release*, p. ej.) que el menor crezca sin cambios de producto y confunda más de lo que ordena — entonces se rediscute la edición, con este precio ya pagado delante |
| D25 | **EL CANAL DE LOS PAQUETES: un repositorio `apt` ESTÁTICO Y FIRMADO, publicado junto a las releases en SourceForge, con la clave pública en un quinto paquete `encina-keyring`. Decisión de Jorge, 2026-08-31** (*«Adelante con la recomendación»*, eligiendo la opción **(b)** de la tabla de [tareas/actualizacion.md](tareas/actualizacion.md), bloque C, con su precio delante). **Lo que compra:** un `sudo apt full-upgrade` trae la versión siguiente a las máquinas instaladas; la obligación de D5 ante un fallo de seguridad de un paquete se cumple **sin reinstalar**; y **los `.deb` del canal son, por huella, los mismos bytes de la release publicada** — la reproducibilidad de §4.39 sigue siendo el argumento del producto, ahora también del canal. **Lo que cuesta, sin maquillar:** una clave de firma que custodiar (dónde vive, quién la tiene y cómo se rota se escribe en la casilla C2, no aquí), un quinto paquete con nombre propio —`encina-keyring`, porque una clave y una fuente propias son una responsabilidad nueva y no caben en `encina-firefox-native` ni en `encina-branding`—, y una medición pendiente ANTES de ir en vivo: que `apt` siga el `302` de SourceForge (C3). **Y una regla de orden que nace medida, no supuesta:** el repositorio se publica —aunque esté vacío— **antes** de que ninguna máquina lleve `encina-keyring` puesto, porque una fuente de `apt` cuyo servidor no contesta convierte cada `apt update` en un error rc 100 (es el patrón de B2, §4.84e, aplicado al canal) | **Por qué (b) y no (a) ni (c), con las tres opciones y precios que ya estaban escritos en el bloque C.** **(a) «ninguna, declarada»** era honesta pero convierte cada fallo de seguridad en «corta un medio y reinstala» —el camino entero de A3 (§4.86), ~20 min solo de subida— para cada máquina instalada, y deja el arreglo medido de C-bis (§4.85) sin camino hacia las máquinas existentes que no sea la orden a mano. **(c) el PPA** resuelve firma y alojamiento a cambio de romper el argumento central: Launchpad **construye** los `.deb` desde fuente, así que los bytes del canal dejarían de ser los de la huella publicada, y `comprobar-propios.sh` dejaría de poder arbitrar el canal contra el manifiesto. **(b)** conserva las dos cosas que este proyecto ya sabe hacer y medir: ficheros estáticos en SourceForge (sirviendo con `302`, medido en §4.82h) y `.deb` por huella. **La enmienda a D5 va al lado de D5**, fechada, como su casilla exige. **Qué reabriría D25:** que SourceForge deje de servir estáticos de forma fiable, que la medición del `302` de C3 salga mal (se mide antes de ir en vivo, no después), o que el proyecto gane una infraestructura mejor que un directorio estático — hasta entonces, lo simple que ya está medido gana a lo potente sin medir |
...
| **E6** | amd64 | Cuando haya con qué probarlo (D9). Repetir el positivo de extremo a extremo allí | ~~Sin abrir~~ **HECHO, y es la mitad del producto desde el 2026-08-30 (D9 enmendada).** Abierto el 2026-08-22 al aparecer el hierro (§4.64: no hizo falta constructor `amd64`, era una deducción falsa); el medio se reproduce desde §4.74 (`gpt_disk_guid=` fijo); `3d5d12a9…` con 0.1.17 (§4.77) instalado **sin red** en el Acer Aspire ES1-524, 65 / 0 dentro y mirado por Jorge (§4.78); publicado el 2026-08-29 junto al `arm64` (§4.82j-m). *Esta celda decía «Sin abrir» ocho días después de estar hecho: se enmienda el 2026-08-30 al corregir el README* |
