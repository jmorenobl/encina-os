...
| **[actualizacion.md](tareas/actualizacion.md)** | **Actualizar Encina OS: los tres relojes** —base, paquetes, medio— y lo que hay que medir antes de tocar nada. Abierto el 2026-08-30, el día siguiente a publicar, a pregunta de Jorge | **16, en cuatro bloques, y ninguna bloquea nada:** A (delimitar: la regla de numeración como decisión `D`, el README diciendo cómo se actualiza y cómo no, la receta de «sacar `0.2.2`»); B (medir lo que ya existe: `unattended-upgrades` contra Firefox, la fuente local `trusted=yes`, **el salto de LTS contra el anclaje**, y los cuatro `.deb` sobre 26.04); C (el canal de los paquetes: hoy **no hay ninguno** —`/srv/encina-repo` es local y estático—, y la recomendación es un repositorio `apt` firmado con un quinto paquete `encina-keyring`); D (el salto de LTS, sólo con lo que B diga). Cuatro deducciones escritas con su etiqueta y **ninguna medida**. Candidato a E7 de `ENCINA-OS.md` §6, no inscrito |
