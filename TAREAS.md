...
| [refactorizacion.md](tareas/refactorizacion.md) | Ordenar lo que ya funciona: un solo vocabulario, documentos que se puedan leer, **y una organización de proyecto de distribución que aguante que la mire alguien de fuera** | **13 abiertas de 16 desde la noche del 2026-08-23**, y aquí ponía ~~15~~ y luego ~~14~~: **la tarea 13 está HECHA** —la CI construye las dos arquitecturas y las tres huellas de arm64 son las del manifiesto, `MEDICIONES.md` §4.69—; **la tarea 1 está HECHA** — el instrumento existe, es [`bancos/enlaces.sh`](bancos/enlaces.sh), corre en dos segundos y su medición es `MEDICIONES.md` §4.66. Encontró **siete referencias rotas y no dos**, cinco de ellas **fuera del subespacio `§4.x`** que la lectura del día anterior había mirado, y **siete trampas y no tres** — las tres últimas (que una referencia **tachada**, un `§` **dentro de un bloque de código** y **el propio guion** no son punteros) **sólo se ven usándolo**, y sin ellas el comprobador castigaba justo el método de este repositorio: dejar al lado lo que se creía. Y aquí ponía ~~12~~ y antes ~~10~~: **eran ONCE** —once secciones y once casillas, contadas con `grep`; el encabezado del fichero llevaba mal el número desde que se escribió—, doce con la **12** que pidió Jorge. **Y DIECISÉIS DESDE LA TARDE DEL 2026-08-23, PORQUE LA 12 SE EJECUTÓ** —cerrada junto con la 1, que son las dos de ese día—**:** [organizacion-comparada.md](tareas/organizacion-comparada.md), **dieciocho** filas, ninguna sin veredicto, **ocho rechazadas enteras** —así que su control pasa: no se copió—. Nacen la **13** (la CI construye también `arm64`, que es el producto que D9 declara y hoy **no** se construye en CI), la **14** (una sola fuente de la versión: el manifiesto pincha `autofirma 1.9.1+encina4` y los `.md` dicen `+encina2` **en 25 sitios**), la **15** (el constructor de los `.deb`, versionado) y la **16** (la hoja de los `[OJOS]` debidos). **Las once viejas se releyeron ejecutando y NO SE CAE NINGUNA**, con nueve cifras enmendadas en su sitio. **FASE 2: se ADELANTA a publicar y se hace ENTERA**, sin la excepción de las cinco |
...
| [sueltas.md](tareas/sueltas.md) | De un rato cada una, sin bloque | 6 — **la nueva es del hierro, 2026-08-23: en AMD el saludador nace sobre `simpledrm` y muere al llegar `amdgpu`, 0 de 5; remedio medido 3 de 3 (§4.70b)** |
