...
- [x] **La ISO es la oficial reempaquetada, y se demuestra.** **REESCRITA el
      ...
      **ninguno perdido**. **ACTUALIZADA EL 2026-08-15 por D23, y otra vez hacia
      ...
      añadidos incluyen además **la capa de marca**, que desde el 2026-08-20 se
      llama **`/casper/minimal.standard.live.encina.squashfs`** —antes
      `zz-encina.squashfs`— porque `casper` **construye la cadena de capas
      quitando puntos del nombre** y **hace `panic` si un eslabón no existe**
      (§4.58b): el nombre no se elige, se hereda. Y el `grub.cfg` gana además el
      **`layerfs-path=`** que la monta: sin él la capa viaja en el medio y **no
      se monta nunca**, en silencio.
...
| `imagen/fabricar-iso.sh` | **Escrito el 2026-08-10, y ampliado el mismo día.** Construye la ISO a partir de la oficial ~~**añadiendo seis ficheros —el seed y el repo local— y modificando dos**~~ **—esa cuenta se quedó rancia y valió hasta el 2026-08-14—**. **Desde el 2026-08-15 (D23) añade también la capa de marca `/casper/minimal.standard.live.encina.squashfs` (renombrada el 2026-08-20: el nombre es la CADENA de `casper`, §4.58b) y modifica TRES ficheros nombrados: `boot/grub/grub.cfg` (el `locale=` del instalador **y el `menuentry`**), `/.disk/info` y `md5sum.txt` (su precio, §4.21d, ahora con una línea más para que la integridad del medio cubra la capa).** **Y desde el 2026-08-17 (§4.53) cambia una cosa que NO es un fichero: el `Volume id`** —`Ubuntu 24.04.4 LTS arm64` → `Encina OS 0.2.1 arm64`—, **derivado de `marca/disk-info` y no escrito a mano**, con un paso 11 propio que **existe porque el paso 10 es ciego a esto**: el paso 10 compara fichero a fichero y el nombre del volumen vive en los descriptores. Comprueba las huellas de los tres binarios firmados antes y después, compara **todas las entradas del medio** contra la oficial —531 en `1224b5b1…`, y **una más** desde que viaja la capa—, verifica las líneas de `md5sum.txt` contra la ISO construida, y **se niega** si algo no cuadra, como `fabricar-seed.sh`. **Y desde el 2026-08-13 fija también el MODO de lo que añade** —`0644` los ficheros, `0755` el directorio, con un guardián que comprueba que el `chmod` se aplicó (trampa 13)—, porque `cp` heredaba el modo del disco del Mac y ese modo viajaba dentro de la ISO (§4.36k). **Es reproducible, y ya no es una afirmación: cinco construcciones dieron la misma huella, una de ellas desde un directorio con cuatro ficheros fuera de `0644` a propósito (§4.39f)** |
